...
type Product struct {
	...
	Price              int      `json:"price"`    // cents (minor currency units)
	Quantity           int      `json:"quantity"` // stock quantity
	Rank               int      `json:"rank"`     // lower = shown first (merchant-controlled)
	ImageURL           string   `json:"image_url,omitempty"`
	Description        string   `json:"description,omitempty"`
	UsageType          string   `json:"usage_type,omitempty"`          // "intensive", "occasional", "versatile"
	AvailableCountries []string `json:"available_countries,omitempty"` // empty = worldwide
}
...
// featuredSlugs are products that get rank 10 (featured). Others get rank 100.
...
func initCatalog(seed int64) {
	...
	catalog = make([]Product, 0, len(templates))
	for i, t := range templates {
		factor := 0.80 + r.Float64()*0.40 // 0.80 to 1.20
		price := int(math.Round(float64(t.price) * factor))
		qty := 10 + r.Intn(191) // 10-200
...
// categoryCount returns a list of categories with their product counts from a given slice.
...
// filterCatalog filters catalog by category, brand, query (title substring), usageType, and country.
func filterCatalog(category, brand, query, usageType, country string) []Product {
	...
	for _, p := range catalog {
		...
		if country != "" && len(p.AvailableCountries) > 0 {
			if !containsCountry(p.AvailableCountries, country) {
				continue
			}
		}
