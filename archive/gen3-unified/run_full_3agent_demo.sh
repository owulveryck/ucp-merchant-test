#!/bin/bash
# Démo complète avec système 3-agent + Agent Acheteur
...
echo -e "${BLUE}╔══════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║    Démo Complète - 3 Agents + Agent Acheteur    ║${NC}"
echo -e "${BLUE}╚══════════════════════════════════════════════════╝${NC}"
echo ""
...
go run ./demo/cmd/client --obs-url http://localhost:9002 --project $GOOGLE_CLOUD_PROJECT > logs/client.log 2>&1 &
CLIENT_PID=$!
sleep 2

if ! ps -p $CLIENT_PID > /dev/null 2>&1; then
    echo -e "${RED}❌ Client Agent n'a pas démarré${NC}"
    echo -e "${YELLOW}Logs du Client Agent:${NC}"
    cat logs/client.log
    echo ""
    echo -e "${YELLOW}Note: Le Client Agent peut échouer si les credentials Google Cloud ne sont pas configurés${NC}"
    echo -e "${YELLOW}Les autres services continuent de fonctionner normalement.${NC}"
    CLIENT_PID=""
else
...
echo "$OBS_PID" >> .pids
echo "$ARENA_PID" >> .pids
[ -n "$CLIENT_PID" ] && echo "$CLIENT_PID" >> .pids
...
echo "│  🛒 Agent Acheteur actif !                               │"
echo "│     L'agent va acheter automatiquement chez le moins     │"
echo "│     cher avec le code AUTO_COMPETE                       │"
echo "│                                                           │"
fi
...
tail -f logs/arena.log 2>/dev/null | grep --line-buffered -E "Agent|ArenaAdapter|3-AGENT" &
TAIL_PID=$!
