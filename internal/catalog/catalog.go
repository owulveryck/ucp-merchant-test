...
// Catalog holds the product catalog.
type Catalog struct {
	Products   []Product
	ProductSeq int
}
...
func (c *Catalog) Find(id string) *Product {
...
// Filter filters products by category, brand, query, usageType, and country.
func (c *Catalog) Filter(category, brand, query, usageType, country string) []Product {
	var result []Product
...
func (c *Catalog) CategoryCount() []map[string]interface{} {
...
// LoadFromFile loads a product catalog from a JSON file.
...
// Init initializes the catalog with random prices from templates.
func (c *Catalog) Init(seed int64) {
	r := rand.New(rand.NewSource(seed))

	c.Products = make([]Product, 0, len(templates))
